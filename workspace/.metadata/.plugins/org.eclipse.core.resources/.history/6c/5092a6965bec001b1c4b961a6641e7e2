...
window.onload = function fetching() {
    console.log("inside");
    (fetch(`http://localhost:8080/H2HBABBA2903/fetch.do?start=${page}&limit=${table_limit}`)
...
function insertRowinTable(i, ele) {
    ...
    row.insertCell(1).innerHTML = ele.name_customer;
    row.insertCell(2).innerHTML = ele.cust_number;
    row.insertCell(3).innerHTML = invoice_id;
    row.insertCell(4).innerHTML = ele.total_open_amount;
    row.insertCell(5).innerHTML = ele.due_in_date;
    row.insertCell(6).innerHTML = ele.clear_date_prediction;
